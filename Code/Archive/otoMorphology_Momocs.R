...
if (T) {
  ...
  # Compute Q1 and Q3 distances for each point across the entire dataset of each watershed
  ...
  # Calculate the direction (angle) of each point from the center
  coo_df <- coo_df %>%
    group_by(watershed, point_id) %>%
    mutate(
      angle = atan2(mean(Y), mean(X))  # Calculate mean angle for each point across all samples
    )
  
  # Merge Q1 and Q3 distances with angles
  quartile_points <- summary_by_watershed %>%
    left_join(coo_df %>% select(watershed, point_id, angle) %>% distinct(), 
              by = c("watershed", "point_id")) %>%
    mutate(
      Q1_X = Q1_distance * cos(angle),  # Project Q1 distance onto x-axis
      Q1_Y = Q1_distance * sin(angle),  # Project Q1 distance onto y-axis
      Q3_X = Q3_distance * cos(angle),  # Project Q3 distance onto x-axis
      Q3_Y = Q3_distance * sin(angle)   # Project Q3 distance onto y-axis
    )
  ...
  mean_shape_by_watershed <- coo_df %>%
    ...
    summarize(
      ...
      mean_Y = median(Y, na.rm = TRUE),
      .groups = "drop"
    )
  
  # Plot all individual outlines as transparent grey lines
  ggplot() +
    # Plot all individual outlines (transparent and grey)
    geom_path(data = coo_df, aes(x = X, y = Y, group = picname), 
              color = "grey", alpha = 0.3, size = 0.5) +
    # Plot Q1 line
    geom_path(data = quartile_points, aes(x = Q1_X, y = Q1_Y, color = "Q1"), size = .6) +
    # Plot Q3 line
    geom_path(data = quartile_points, aes(x = Q3_X, y = Q3_Y, color = "Q3"), size = .6) +
    # Add mean shape per watershed as a line
    geom_path(data = mean_shape_by_watershed, aes(x = mean_X, y = mean_Y, group = watershed), 
              color = "black", size = .6, linetype = "solid") +
    facet_wrap(~ watershed, ncol = 1) +
    coord_equal() +
    labs(title = "Otolith Shape Variation with Q1 and Q3 Lines (Per Watershed)", 
         x = "X Coordinate", y = "Y Coordinate") +
    scale_color_manual(values = c("Q1" = "dodgerblue4", "Q3" = "firebrick"), name = "Quartile") +
    theme_minimal()
}
