...
process_trimmed_data <- function(interp_points = 1000, window_size = 60, gamma_value = 1.4, landmark_filter = c("Core", "Fw")) {     
  ...
  results_list <- pblapply(files, function(file_path) {         
    ...
    # if the difference is less than specified, skip 
    ...
    if ("Fw" %in% landmark_filter) {
      ...
      # Collect all reads within the specified landmarks
      ind_data <- ind_data %>% 
  ...
  write.csv(all_data_combined_raw, file = here(paste0("Data/Classification_ts_matrices/Sr8786/", filename_base, "_RAW.csv")), row.names = FALSE)
  ...
  write.csv(all_data_combined_MA, file = here(paste0("Data/Classification_ts_matrices/Sr8786/", filename_base, "_MA.csv")), row.names = FALSE)
  
  # Save GAM smoothed data
  all_data_combined_GAM <- cbind(metadata, gam_smoothed_array)
  write.csv(all_data_combined_GAM, file = here(paste0("Data/Classification_ts_matrices/Sr8786/", filename_base, "_GAM.csv")), row.names = FALSE)
