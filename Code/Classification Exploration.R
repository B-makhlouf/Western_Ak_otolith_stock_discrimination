...
# AnalysisDataAll2 <- processed_data2 %>%
#   left_join(All_Metadata %>% select(-Year), by = c("Fish_id" = "Fish_ID")) %>%
#   select((ncol(.)-12):ncol(.), everything()) %>%
#   filter(QC_Grade == "Yes")
...
#Analysis_metadata2<- AnalysisDataAll2[,1:17] #Seperate Metadata
Analysis_ts_data<- AnalysisDataAll[,18:length(AnalysisDataAll)] #Seperate Isotope ts data 
#Analysis_ts_data2<- AnalysisDataAll2[,18:length(AnalysisDataAll2)] #Seperate Isotope ts data

#combine both ts data
#Analysis_ts_data<- cbind(Analysis_ts_data, Analysis_ts_data2)
...
ModelData <- Analysis_ts_data %>% as.data.frame() 

#Rename the column names 1:2000
colnames(ModelData) <- c(1:ncol(ModelData))

ModelData<- ModelData%>% mutate(Watershed = Analysis_metadata$Watershed)
...
# Make sure Watershed is a factor in both training and testing data
...
idScores <- Analysis_metadata[-trainIndex, ] %>%
  select(Fish_id) %>%
  mutate(
    Predicted = predictions,   # Use default predicted class labels
...
# Add metadata to idScores by Fish_ID 
idScores <- left_join(idScores, All_Metadata, by = c("Fish_id" = "Fish_ID"))

# Optionally, check the class distribution in test data
#table(testdata$Watershed)
# 
# calculate_specificity <- function(conf_matrix) {
#   TN <- conf_matrix$table[1, 1]  # True Negatives (bottom-right for binary)
#   FP <- conf_matrix$table[1, 2]  # False Positives (top-right for binary)
#   specificity <- TN / (TN + FP)
#   return(specificity)
# }
# 
# # Initialize a list to store specificity results
# specificity_results <- list()
# 
# # Compute specificity for each class (vs. all others)
# for (class_label in unique(idScores$Actual)) {
#   
#   # Convert to binary classification: "Class of interest" vs. "Not Class of interest"
#   binary_actual <- ifelse(idScores$Actual == class_label, class_label, "Not")
#   binary_predicted <- ifelse(idScores$Predicted == class_label, class_label, "Not")
#   
#   # Compute confusion matrix for binary classification
#   binary_conf_matrix <- confusionMatrix(factor(binary_predicted), factor(binary_actual))
#   
#   # Calculate specificity
#   specificity_results[[class_label]] <- calculate_specificity(binary_conf_matrix)
# }
# 
