...
preprocessed_data<- process_trimmed_data() 

metadata <- tibble(
  Fish_id = preprocessed_data$ids,
  Watershed = preprocessed_data$watersheds,
  Natal_Iso = preprocessed_data$natal_origins,
  Year = preprocessed_data$Year
)

iso_data_raw<- preprocessed_data$measurement_array #RAW interpolated data 
iso_data_MA<- preprocessed_data$moving_avg_array # Moving average data
iso_data_MA <- iso_data_MA[, colSums(is.na(iso_data_MA)) == 0] # MA has tails of NA, remove
iso_data_GAM<- preprocessed_data$gam_smoothed_array # GAM smoothed data
