...
process_trimmed_data <- function(interp_points = 1000, window_size = 60, gamma_value = .8, landmark_filter = c("Core", "Fw")) {
  ...
  for (file in landmark_files) {
    ...
    # Interpolate ind_data_extended to 1000 reads 
    interpolated_GAM <- approx(seq_len(nrow(ind_data_extended)), ind_data_extended$GAM_smoothed, seq(1, nrow(ind_data_extended), length.out = interp_points), method = "linear", rule = 2)$y
    
    # Interpolate the MA 
    interpolated_MA <- approx(seq_len(nrow(ind_data_extended)), ind_data_extended$Moving_Avg, seq(1, nrow(ind_data_extended), length.out = interp_points), method = "linear", rule = 2)$y
    
    # Interpolate the Sr88
    interpolated_Sr88 <- approx(seq_len(nrow(ind_data_extended)), ind_data_extended$Moving_Avg_Sr88, seq(1, nrow(ind_data_extended), length.out = interp_points), method = "linear", rule = 2)$y
    
    # Interpolate the Combined
    interpolated_Combined <- approx(seq_len(nrow(ind_data_extended)), ind_data_extended$Combined, seq(1, nrow(ind_data_extended), length.out = interp_points), method = "linear", rule = 2)$y
