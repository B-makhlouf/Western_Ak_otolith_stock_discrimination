...
# Extract the ordered natal iso values
...
# Remove these indices from all data, unless there's an NA, then just change the name and keep the data 
...
# Z-normalize the Iso values for each individual separately
measurement_array_normalized <- apply(measurement_array_filtered, 1, function(x) {
  # Calculate the mean and standard deviation for each individual (row)
  individual_mean <- mean(x, na.rm = TRUE)
  individual_sd <- sd(x, na.rm = TRUE)
  
  # If the standard deviation is not zero, normalize; otherwise, keep the original values
  if (individual_sd != 0) {
    return((x - individual_mean) / individual_sd)
  } else {
    return(rep(NA, length(x)))  # Return NA if SD is zero
  }
})

# Convert the result back to a data frame or tibble if necessary
measurement_array_normalized <- t(measurement_array_normalized)

# You can now proceed to PCA analysis on the `measurement_array_normalized` or continue with the next steps of your analysis

# Perform PCA on the normalized Iso values
results <- prcomp(measurement_array_normalized, center = TRUE, scale. = TRUE)
